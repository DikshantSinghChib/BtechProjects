...
function addProcess() {
    ...
    let rowHTML1 = `
        ...
        <td class="arrival-time" rowspan="2"><input type="number" min="0" step="1" value="0" id="ar${process}" class="box"></ TDS>
...
function compareAlgorithms() {
    try {
        ...
        [primaryAlgo, secondaryAlgo].forEach(algo => {
            ...
            section.style.minWidth = "45%";
            section.innerHTML = `<h4>${algo.toUpperCase()}</h4>`;
            
            // Show Gantt Chart
            showGanttChart(output, section);
            
            // Show Timeline Chart
            showTimelineChart(output, section);
